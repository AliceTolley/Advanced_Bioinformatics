...
trimmomatic PE -threads 4 \
 -phred33 /home/ubuntu/ngs_course/m2305732_assess/data/untrimmed_fastq/NGS0001.R1.fastq.gz \
...
#Remove redundant zipped file to save space
rm -r hg19.fa.gz

#Build reference index, "-t 4" assigns 4 threads to speed up the process
bowtie2-build -t 4 hg19.fa hg19

#Make aligned_data directory
mkdir ~/ngs_course/m2305732_assess/data/aligned_data

#Run alignment: "-x" to denote reference file, "-1" for read pair 1, "-2" for read pair 2, "-S" to denote the name and location of output file
bowtie2 -p 4 -x ~/ngs_course/m2305732_assess/data/reference/hg19 \
 -1 ~/ngs_course/m2305732_assess/data/trimmed_fastq/NGS0001_trimmed_R_1P \
 -2 ~/ngs_course/m2305732_assess/data/trimmed_fastq/NGS0001_trimmed_R_2P \
 -S ~/ngs_course/m2305732_assess/data/aligned_data/NGS0001_bowtie.sam
...
#Convert SAM to BAM
samtools view -h -b NGS0001_bowtie.sam > NGS0001_bowtie.bam

#Sort BAM file
samtools sort NGS0001_bowtie.bam > NGS0001_bowtie_sorted.bam

#Generate index from sorted BAM file
samtools index NGS0001_bowtie_sorted.bam

##Markduplicates
picard MarkDuplicates I=NGS0001_bowtie_sorted.bam O=NGS0001_bowtie_sorted_marked.bam M=marked_dup_metrics.txt

samtools index NGS0001_bowtie_sorted_marked.bam

##Filter SAM OR BAM
samtools view -F 1796  -q 20 -o NGS0001_bowtie_sorted_filtered.bam NGS0001_bowtie_sorted_marked.bam

samtools index NGS0001_bowtie_sorted_filtered.bam

##Flagstat
samtools flagstat NGS0001_bowtie_sorted_filtered.bam

##IdxStats
samtools idxstats NGS0001_bowtie_sorted_filtered.bam

##Depth of Coverage
bedtools coverage -a NGS0001_bowtie_sorted_filtered.bam -b ~/ngs_course/m2305732_assess/data/annotation.bed > Depth_of_Coverage.txt

##CollectInsertSizeMetrics
picard CollectInsertSizeMetrics I=NGS0001_bowtie_sorted_filtered.bam O=insert_size_metrics.txt H=insert_size_metrics.pdf M=0.5
#############################################################################################################################
...
freebayes --bam ~/ngs_course/m2305732_assess/data/aligned_data/NGS0001_bowtie_sorted_filtered.bam \
 --fasta-reference ~/ngs_course/m2305732_assess/data/reference/hg19.fa \
 --vcf ~/ngs_course/m2305732_assess/results/NGS0001_bowtie.vcf

#Zip vcf file to save space
bgzip ~/ngs_course/m2305732_assess/results/NGS0001_bowtie.vcf

tabix -p vcf ~/ngs_course/m2305732_assess/results/NGS0001_bowtie.vcf.gz
...
vcffilter -f "QUAL > 1 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1" \
 ~/ngs_course/m2305732_assess/results/NGS0001_bowtie.vcf.gz > ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_filtered.vcf

#Filter VCF to the regions in the bed file
bedtools intersect -header -wa -a ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_filtered.vcf \
 -b ~/ngs_course/m2305732_assess/data/annotation.bed > ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.vcf

#Zip filtered VCF file
bgzip ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.vcf

tabix -p vcf ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.vcf.gz
...
~/ngs_course/m2305732_assess/annovar/annovar/convert2annovar.pl -format vcf4 \
 ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.vcf.gz > \
 ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.avinput

#Move into Annovar directory
cd ~/ngs_course/m2305732_assess/annovar/annovar

#Run Annovar table function to output a csv file
./table_annovar.pl ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.avinput humandb/ \
 -buildver hg19 -out ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered -remove \
 -protocol refGene,ensGene,clinvar_20180603,exac03,dbnsfp31a_interpro -operation g,g,f,f,f -otherinfo -nastring . -csvout

#OUTPUT LOCATION: /home/ubuntu/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.hg19_multianno.csv

##Filter for variants not in dbsnp
./annotate_variation.pl -filter -out ~/ngs_course/m2305732_assess/results/NGS0001bowtie \
 -build hg19 -dbtype snp138 ~/ngs_course/m2305732_assess/results/NGS0001_bowtie_bed_filtered.avinput humandb/

#OUTPUT LOCATION: /home/ubuntu/ngs_course/m2305732_assess/results/NGS0001bowtie.hg19_snp138_filtered
#Above output contains variants not found in dbsnp

#Convert dbsnp filtered file to csv file
./table_annovar.pl ~/ngs_course/m2305732_assess/results/NGS0001bowtie.hg19_snp138_filtered humandb/ \
 -buildver hg19 -out ~/ngs_course/m2305732_assess/results/NGS0001bowtie.hg19_snp138_filtered -remove \
 -protocol refGene,ensGene,clinvar_20180603,exac03,dbnsfp31a_interpro -operation g,g,f,f,f -otherinfo -nastring . -csvout

#Transfer csv file to personal device using filezilla, filter in excel for exonic variants.
#Viewable as excel file uploaded to Canvas: "NGS0001bowtie.hg19_snp138_filtered.hg19_multianno.csv"
...
#Unzip hg19 database: hg19 database is unzipped into data directory within snpEff
...
java -Xmx4G -jar snpEff/snpEff.jar -i vcf -o vcf hg19 NGS0001_bowtie_bed_filtered.vcf.gz > NGS0001_bowtie_Snpeff.vcf
